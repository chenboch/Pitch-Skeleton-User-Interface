...
from ..utils import (FPSTimer, OneEuroFilter)
...
class PoseLifter(object):
    ...
    def detect_keypoints(self, image:np.ndarray, frame_num:int = None):
        ...
        if frame_num not in self.processed_frames:
            if frame_num % 3 == 0:
                ...
                # self.fps_timer.toc()
                # print(f"detect time: {self.fps_timer.time_interval}, fps: {int(self.fps_timer.fps) if int(self.fps_timer.fps)  < 100 else 0}")
                # print(prof.key_averages().table(sort_by="cuda_time_total", row_limit=10))
                self._bbox_buffer = bboxes
            else:
                bboxes = self._bbox_buffer
            # with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA]) as prof:
            # self.fps_timer.tic()
            ...
            # with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA]) as prof:
            # self.fps_timer.tic()
            pose_results, pred_instances = self.pose2d_estimator.process_image(image, online_bbox)
            # self.fps_timer.toc()
            # print(f"pose2d time: {self.fps_timer.time_interval}, fps: {int(self.fps_timer.fps) if int(self.fps_timer.fps)  < 100 else 0}")
            # print(prof.key_averages().table(sort_by="cuda_time_total", row_limit=10))
            # print(prof.key_averages().table(sort_by="cuda_time_total", row_limit=10))
    ...
    def person_df(self, load_df:pl.DataFrame):
        if load_df.is_empty():
            self.logger.info(f"讀取資料的狀態: { not load_df.is_empty()}")
            return
