...
from ..backbones.vision_transformer_token import VisionTransformerToken
...
class DSTA_STD_VitM(BaseModel):

    def __init__(self, cfg, phase, **kwargs):
        super(DSTA_STD_VitM, self).__init__()
        self.logger = logging.getLogger(__name__)
        self.pretrained_layers = cfg['MODEL']['EXTRA']['PRETRAINED_LAYERS']
        self.is_train = True if phase == TRAIN_PHASE else False
        self.freeze_hrnet_weights = cfg.MODEL.FREEZE_HRNET_WEIGHTS
        self.num_joints = cfg.MODEL.NUM_JOINTS
        self.preact = VisionTransformerToken('base')

        self.pretrained = cfg.MODEL.PRETRAINED
        self.embed_dim_ratio = 192
        ...
        # nets = nn.Sequential(nn.Linear(2, 64), nn.LeakyReLU(), nn.Linear(64, 64), nn.LeakyReLU(), nn.Linear(64, 2), nn.Tanh())
        # nett =  nn.Sequential(nn.Linear(2, 64), nn.LeakyReLU(), nn.Linear(64, 64), nn.LeakyReLU(), nn.Linear(64, 2))
        # masks = torch.from_numpy(np.array([[0, 1], [1, 0]] * 3).astype(np.float32))
        # prior = distributions.MultivariateNormal(torch.zeros(2) + 0.5, torch.eye(2))
        self.flow = RealNVP()
        # self.avg_pool = nn.AdaptiveAvgPool2d(1)
        # self.fc_coord = nn.Linear(768, self.num_joints)
        self.fc_coord = Linear(768, self.num_joints)
    ...
    def forward(self, x, meta=None):
        ...
        # feat = feat.reshape(batch_size * self.num_frame, -1)
        # feat = rearrange(feat, 'b c n -> b n c')
        # print(feat.shape)
        feat = self.fc_coord(feat)
        feat = rearrange(feat, 'b c n -> b n c')
        # print(feat.shape)
        # exit()
    ...
    def get_net(cls, cfg, phase, **kwargs):
        model = DSTA_STD_VitM(cfg, phase, **kwargs)
        return model
