...
class CommonFunction(BaseFunction):
    ...
    def __init__(self, cfg, **kwargs):
        super().__init__()
        self.cfg = cfg
        self.hm_shape = cfg.MODEL.HEATMAP_SIZE
        ...
        self.PE_Name = kwargs.get("PE_Name", "DSTA")
        ##
        self.max_iter_num = 0
    ...
    def train(self, model, epoch, optimizer, dataloader, tb_writer_dict, **kwargs):
        ...
        for iter_step in range(self.max_iter_num):
            input_prev, input_x, input_nxt, meta = next(self.dataloader_iter)
            concat_input = torch.cat((input_prev, input_x, input_nxt), 1).cuda()
            output = model(concat_input, meta)
            loss = self.criterion(output, meta['target'].cuda(), meta['target_weight'].cuda())
            acc = output.acc
            num_images = len(input_x)
            loss_meter.update(loss.item(), num_images)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            acc_meter.update(acc, num_images)

            if iter_step % self.cfg.PRINT_FREQ == 0 or iter_step >= self.max_iter_num - 1:
                msg = 'Epoch:[{0}][{1}/{2}] {loss}{acc}lr:{lr}'.format(
                    epoch, iter_step, self.max_iter_num,
                    loss=self._get_loss_info(loss_meter, 'Loss'),
                    acc=self._get_loss_info(acc_meter, 'Acc'),
                    lr=round(optimizer.param_groups[0]['lr'], 7)

                )
                logger.info(msg)
            self.tb_writer.add_scalar('loss', loss_meter.val, self.global_steps)
            self.global_steps += 1
        tb_writer_dict["global_steps"] = self.global_steps

    def _get_loss_info(self, meter, loss_name):
        msg = '{name}:{meter.val:.3f}({meter.avg:.3f}) '.format(name=loss_name, meter=meter)
        return msg
    ...
    def eval(self, model, dataloader, tb_writer_dict, **kwargs):
        ...
        with torch.no_grad():
            ...
            for iter_step in range(self.max_iter_num):
                input_prev, input_x, input_nxt, meta = next(self.dataloader_iter)
                concat_input = torch.cat((input_prev, input_x, input_nxt), 1).cuda()
                r = model(concat_input)
                pred_coor = r.pred_jts.detach().cpu().numpy()
                score_coor = r.maxvals.detach().cpu().numpy()
                if phase == VAL_PHASE:
                ...
                if iter_step % self.cfg.PRINT_FREQ == 0 or iter_step >= (num_batch - 1):
                    msg = '{}: [{}/{}]'.format(phase, iter_step, num_batch)
                    logger.info(msg)
                ...
                score_box = meta['score'].numpy()
                num_images = input_x.size(0)

                pred_coor = pred_coor * [3, 4]
                preds, maxvals = get_final_preds_coor(pred_coor, score_coor, center, scale, h=4, w=3)
                ...
                all_boxes[idx:idx + num_images, 5] = score_box
