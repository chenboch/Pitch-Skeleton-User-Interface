...
from DSTA_main.utils.utils_json import read_json_from_file, write_json_to_file
from DSTA_main.utils.utils_bbox import box2cs
from DSTA_main.utils.utils_image import read_image
from DSTA_main.utils.utils_folder import create_folder
from DSTA_main.utils.utils_registry import DATASET_REGISTRY
...
from DSTA_main.utils.common import TRAIN_PHASE, VAL_PHASE, TEST_PHASE
...
class PoseTrack(VideoDataset):
    ...
    def __init__(self, cfg, phase, **kwargs):
        ...
        # self.joints_weight = np.array([1., 1., 1., 1., 1., 1., 1., 1.2, 1.2, 3., 3., 1., 1., 1.2, 1.2, 3., 3.], dtype=np.float32).reshape((self.num_joints, 1))
    ...
    def _get_spatiotemporal_window(self, data_item):
        ...
        if not is_posetrack18:
            prev_idx = max((current_idx - 1), 1)
            next_idx = min(current_idx + 1, num_frames)
        else:
        ...
        return input_prev, input_x, input_next, meta
