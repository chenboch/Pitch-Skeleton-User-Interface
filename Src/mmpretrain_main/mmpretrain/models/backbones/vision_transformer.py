...
from ..utils import (MultiheadAttention, GatedAttention, SwiGLUFFNFused, build_norm_layer,
...
@MODELS.register_module()
class VisionGAUTransformer(BaseBackbone):
    """Vision Transformer.
    ...
    def __init__(self,
                 ...
                 pre_norm=False,
                 init_cfg=None):
        super(VisionGAUTransformer, self).__init__(init_cfg)
        ...
        for i in range(self.num_layers):
            _layer_cfg = dict(
                num_token=15,
                in_token_dims=self.embed_dims,
                out_token_dims=self.embed_dims,
                expansion_factor=2,
                s=self.embed_dims//4,
                eps=1e-5,
            ...
            _layer_cfg.update(layer_cfgs[i])
            self.layers.append(GatedAttention(**_layer_cfg))
    ...
    def init_weights(self):
        super(VisionGAUTransformer, self).init_weights()
